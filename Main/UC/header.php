<div class="vertical-nav bg-white" id="sidebar">
          <div class="py-4 px-3 mb-2 mt-2 bg-light">
            <div class="align-items-center" id="half">    
               <a class="navbar-brand text-center" href="#"><h1>C M S</h1></a>
            </div>
          </div>
          <p class="text-gray font-weight-bold text-uppercase px-3 small pb-3 mb-2 mt-3">Main</p>
          <ul class="nav flex-column bg-white mb-0">
            <li class="nav-item">
              <a href="index.php" class="nav-link text-dark font-italic bg-light">
                <i class="fa fa-th-large mr-3 text-primary fa-fw"></i>Home
              </a>  
            </li>
            <li class="nav-item">
              <a href="my-profile.php" class="nav-link text-dark font-italic bg-light">
                <i class="far fa-user mr-3 text-primary fa-fw"></i>
                My profile
              </a>
            </li>
            <li class="nav-item">
              <a href="details.php" class="nav-link text-dark font-italic bg-light">
                <i class="far fa-plus mr-3 text-primary fa-fw"></i>
                Add Details
            </a>
            <ul class="nav-item">
            <a href="view-tt.php" class="nav-link text-dark font-italic bg-light">
                <i class="far fa-eye mr-3 text-primary fa-fw"></i>
              View time Table
            </a>
            </ul>
            </li>
            
            <li class="nav-item">
              <a href="view-details.php" class="nav-link text-dark font-italic bg-light">
                <i class="far fa-users mr-3 text-primary fa-fw"></i>
              Details
              </a>
            </li>
            <li class="nav-item">
              <a href="../../access/logout.php" class="nav-link text-dark font-italic bg-light">
                <i class="far fa-sign-out mr-3 text-primary fa-fw"></i>
                Logout
              </a>
            </li>
          </ul>
       </div>