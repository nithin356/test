...
if(isset($_POST['del'])){
    $id=$_POST['del'];
        ...
        $sql = mysqli_query($con, "DELETE FROM lec WHERE id='$id'");
        if ($sql) {
            echo "<script>alert('User Deleted');</script>";
            $smsg = "User Updated";
        } else {
...
<html>
  ...
  <?php
  $gets1 = mysqli_query($con, "SELECT * FROM lec");
  while($getstudent1=mysqli_fetch_assoc($gets1)){
  ?>
  <tr>
    <form method="POST">
    <td><?php echo $getstudent1['id'];?></td>
    <td><?php echo $getstudent1['lname'];?></td>
    <td><?php echo $getstudent1['unit'];?></td>
    <?php if($getstudent1['access']=='0'){?>
            <td>Access Not Allowed</td>
    <?php }else{ ?>
    <td>Access Allowed</td>
    <?php } ?>
    <td><input type="hidden" value="<?php echo $getstudent1['id'];?>" name="del"/>
